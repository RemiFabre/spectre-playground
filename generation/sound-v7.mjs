...
function tone(dst, { at, freq, dur, amp = 0.2, panv = 0, partials = [[1, 1]], attack = 0.004, curve = 4, fm = null, drift = 0 }) {
  ...
  for (let s = 0; s < n && st + s < N; s++) {
    const tt = s / SR;
    let env = (s < attack * SR ? s / (attack * SR) : Math.exp(-curve * (tt - attack) / dur));
    if (s > 0.75 * n) env *= (n - s) / (0.25 * n);
    let fmod = 0;
...
variants["S4e2-final"] = () => {
  const d = buf();
  buildSection(d);
  const cadence = FROM() + 4 * BAR;
  // the groove ends WITH the melody: one last accented hit under the high A5,
  // nothing rhythmic left to stop mid-ring afterwards
  grooveChords(d, { from: summitT + B, to: cadence + 0.01 });
  kick(d, { at: cadence, amp: 0.55, f0: 95, f1: 36, dur: 0.5 });
  tone(d, { at: cadence, freq: hz(33), dur: 1.6, amp: 0.2, partials: [[1, 1], [2, 0.4]], curve: 2 });
  for (const e of ev.remove) {
    if (e.i === T - 1 || t(e.f) > cadence) continue;
    hat(d, { at: q16(t(e.f)), amp: 0.05, dur: 0.03, panv: -tilePan(e.i) });
  }
  let end = playBars(d, FROM(), HEAD);
  end = playBars(d, end, [[76, 0, 80, 0, 83, 0, 0, 0]]);
  const tailEnd = 31.6;
  note(d, end, 81, (tailEnd - end) / 1.35, 0.2);
  note(d, end, 69, (tailEnd - end) / 1.35, 0.09);
  tone(d, { at: end, freq: hz(57), dur: tailEnd - end, amp: 0.08, partials: PIANO, curve: 2 });
  tone(d, { at: t(summitGoneF), freq: hz(33), dur: tailEnd - t(summitGoneF), amp: 0.22, partials: [[1, 1], [2, 0.3]], attack: 0.01, curve: 2.2 });
  tone(d, { at: t(summitGoneF), freq: hz(45), dur: tailEnd - t(summitGoneF), amp: 0.12, partials: PIANO, curve: 2.2 });
  echoVerb(d, 0.16, 0.4);
  return d;
};
{
  ...
  const peak = writeWav(`${HERE}/S4e2-final3.wav`, d);
