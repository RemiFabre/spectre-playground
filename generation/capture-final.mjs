...
let f = 0;
const snap = async () => { await page.evaluate(() => window.__frame()); await page.screenshot({ path: `${dir}/f${pad(f++)}.png` }); };
...
for (let i = 0; i < T - 1; i++) {
  await win(0, i + 1);
  await page.evaluate((i) => window.__pulse(i), i);
